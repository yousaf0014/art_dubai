...
?>
<div>
    <?php echo $this->Form->create('User', array('class'=>'span5 form-horizontal')); ?>
    <div class="well">
    <?php
    echo $this->Form->input($user_display_field, array('label' => false, 'div' => 'col-lg-2', 'placeholder' => 'user','class' => 'form-control input-sm'));
    echo $this->Form->button('filter',array('class'=>'btn btn-primary btn-sm','type' => 'submit'));
    echo $this->Form->end();
    ?>    
    </div>

    <div class="span6 alert alert-info">
    <?php echo __d('acl', 'This page allows to manage users specific rights'); ?>
    </div>
</div>
<table class="table table-bordered table-condensed table-striped" >
    <tr>
    	<?php
    	$column_count = 1;    	
    	$headers = array("", $this->Paginator->sort($user_display_field, "Name"));
    ...
    foreach($users as $user)
    {
        ...
        echo '<td class="col-lg-1 text-center">'.$this->Html->link('<i class="glyphicon glyphicon-lock"></i>', $link, 
            array('class'=>'btn btn-primary btn-sm', 'alt' => $title, 'title' => $title, 'escape' => false)).'</td>';
...
?>
    <h1>Usuário: <?php echo  __d('acl', $user_model_name) . ' : ' . $user[$user_model_name][$user_display_field]; ?></h1>
    <hr>
