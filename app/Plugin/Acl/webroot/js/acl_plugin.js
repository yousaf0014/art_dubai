...
function manage_module_permissions(action,role_id,module_id) {
	app_root_url = ensure_ends_with(app_root_url,'/');
	jQuery('#module_permission_loader_'+role_id).show();
	jQuery.ajax({
		url : app_root_url+'/admin/acl/aros/module_permissions',
		type : 'get',
		data : {action:action,role_id:role_id,module_id:module_id}
	}).done(function(){
		if(action == 'allow'){
			jQuery('#permission_cross_'+role_id).css('display','none');
			jQuery('#permission_tick_'+role_id).css('display','');
		}else{
			jQuery('#permission_cross_'+role_id).css('display','');
			jQuery('#permission_tick_'+role_id).css('display','none');
		}
		jQuery('#module_permission_loader_'+role_id).hide();
	});
}