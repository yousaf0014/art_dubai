...
class ArosController extends AclAppController
{
	...
	function admin_user_permissions($user_id = null)
	{
	    ...
	    else
	    {
            ...
            else
            {
		    	 ...
		    	 */
		    	$count = $this->Aro->Permission->find('count', array(
                    'conditions' => array('Aro.id' => $user_aro[0]['Aro']['id']),
                    'recursive' => 0
                ));
		    	if($count != 0)
		    	{
	...
	public function admin_module_permissions() {
		if($this->request->is('ajax')) {
			...
			$this->AclManager->manage_module_permissions($action, $aro_node, $module_id);
			echo '1';
			exit;
		}
		$modules = $this->Acl->Aro->find('all',array(
			'conditions' => array('model' => 'AcoGroup'),
			'recursive' => '-1'
		));

		App::uses('ModulePermission','Model');
		$modulePermissionObj = new ModulePermission();
		$modulePermissions = $modulePermissionObj->find('list',array(
			'fields' => array('unique_id','module_id')
		));
		App::uses('Role','Model');
		$roleObj = new Role();
		$roles = $roleObj->find('list',array(
			'conditions' => array('active' => '1'),
			'fields' => array('id','name')
		));
		$this->set(compact('modules','roles','modulePermissions'));
	}
}
?>