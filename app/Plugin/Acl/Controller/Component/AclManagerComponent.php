...
//		}
    ...
    */
    ...
    public function getAco($controller,$action){
    	$parentID = $this->Acl->Aco->field('id',array('alias' => $controller));
    	return $this->Acl->Aco->find('first',array(
			'conditions' => array('parent_id' => $parentID, 'alias' => $action),
			'recursive' => -1
		));
    }

    public function manage_module_permissions($action, $aro_node, $module_id) {
    	$acoGroupObj = $this->get_model_instance('AcosGroup');

		$acos = $acoGroupObj->find('all',array(
			'conditions' => array('aro_id' => $module_id),
			'recursive' => '-1'
		));

    	foreach ($acos as $key => $aco) {
    		$this->save_permission($aro_node,$aco['AcosGroup']['path'],$action);
    	}
    	$modulePermissionObj = $this->get_model_instance('ModulePermission');
    	$aro_id = $aro_node[0]['Aro']['foreign_key'];
    	if($action == 'allow') {
    		$modulePermissionObj->save(array('foreign_key' => $aro_id, 'module_id' => $module_id,'model' => 'Role'));
    	}else{
    		$modulePermissionObj->deleteAll(array('foreign_key' => $aro_id, 'module_id' => $module_id, 'model' => 'Role'));
    	}
    }
}